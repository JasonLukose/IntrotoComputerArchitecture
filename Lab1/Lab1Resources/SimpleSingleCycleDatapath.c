...
/*
    Loads byte: DR = BASE REG + Signextention of the offset
    Also sets condition code based the value of INST.DR
*/
 void execute_LDB(){
      NEXT_LATCHES.REGS[INST.DR] = Low16bits(SEXT(read_byte(Low16bits(CURRENT_LATCHES.REGS[INST.BaseR] + SEXT(INST.boffset6,6))),8));
    setcc(NEXT_LATCHES.REGS[INST.DR]);
 
 }

/*
    Store byte: Takes BaseR and uses offset to get value which is written to memory
*/
 void execute_STB(){
    write_byte(Low16bits(CURRENT_LATCHES.REGS[INST.BaseR] + SEXT(INST.boffset6,6)), CURRENT_LATCHES.REGS[INST.DR]);

 }
 
 /*
    Reg7 saves PC value
    Next states PC is dependent on INST.B 
*/
 void execute_JSR(){
    NEXT_LATCHES.REGS[7] = NEXT_LATCHES.PC;
    if(INST.B == 0) {
...
/*
    AND same as ADD but with bitwise AND
    Also sets condition codes based on DR Value
*/ 
 void execute_AND(){
     if (INST.A == 0) {
        NEXT_LATCHES.REGS[INST.DR] = Low16bits(CURRENT_LATCHES.REGS[INST.SR1] & CURRENT_LATCHES.REGS[INST.SR2]); 
...
/* 
    Nearly same as LDB but is a word instead of a byte
    Sets condition code based on DR
*/
 void execute_LDW(){
    NEXT_LATCHES.REGS[INST.DR] = read_word(Low16bits(CURRENT_LATCHES.REGS[INST.BaseR] + LSHF(SEXT(INST.offset6,6),1)));
    setcc(NEXT_LATCHES.REGS[INST.DR]);
 }
/*
    Same as STB but stores byte
*/
 void execute_STW(){
    write_word(Low16bits(CURRENT_LATCHES.REGS[INST.BaseR] + LSHF(INST.offset6,1)), CURRENT_LATCHES.REGS[INST.DR]);


 }

/* 
    Same as ADD but with XOR
    Sets Condition Codes based on INST.DR
*/
...
/*
    Shift: Left, RightShiftA and RightShiftLogical
    RightA and RightL are different and depend on different values to differentiate
*/
 void execute_SHF(){
   if(INST.D == 0) {
        NEXT_LATCHES.REGS[INST.DR] = LSHF(CURRENT_LATCHES.REGS[INST.SR1], INST.amount4);
    } else {
        if(INST.A == 0){
            NEXT_LATCHES.REGS[INST.DR] = RSHF(CURRENT_LATCHES.REGS[INST.SR1], INST.amount4, 0);
        } else {
            NEXT_LATCHES.REGS[INST.DR] = RSHF(CURRENT_LATCHES.REGS[INST.SR1], INST.amount4, get_bits( CURRENT_LATCHES.REGS[INST.SR1] , 15, 15));
        }     
    }
    setcc(NEXT_LATCHES.REGS[INST.DR]);
} 
 /*
    Load Effective Address: PC + Shifting with the offset9
*/
 void execute_LEA(){
