...
int memDRID, memPCMUX, memLDPC, memLDCC, memTRAPPC, memTARGETPC;
int v_mem_ld_reg, v_mem_ld_cc;
...
void MEM_stage() {
  ...
  int dCacheEN = Get_DCACHE_EN(PS.MEM_CS);
  int RW = Get_DCACHE_RW(PS.MEM_CS);
  int WE0, WE1;
  int oddAddress = PS.MEM_ADDRESS & 0x1;
  int dSize = Get_DATA_SIZE(PS.MEM_CS);
  int dataOnWrite = PS.MEM_ALU_RESULT;
  int dataOnRead;
  int dCacheR;

if (dCacheEN & PS.MEM_V) { 
  ...
  if (!RW) {
    ...
    if (dSize) {
        ...
        if (oddAddress) {
            WE1 = 1;
            WE0 = 0;
	    int lowByte = PS.MEM_ALU_RESULT & 0xFF;
	    dataOnWrite = Low16bits(lowByte << 8 | lowByte);
        } else {
            WE0 = 1;
            WE1 = 0;
        }
    ...
    /* Call to DCache */
    dcache_access(PS.MEM_ADDRESS, &dataOnRead, dataOnWrite, &dCacheR, WE0, WE1);
    ...
    if (!dCacheR) {
        ...
        if (!dSize) {
            if (oddAddress) {
                dataOnRead = Low16bits(SEXT((dataOnRead & 0xFF00) >> 8, 8));
            } else {
                dataOnRead =  Low16bits(SEXT(dataOnRead & 0x00FF, 8));
            }   
        } else if (dSize) {
            dataOnRead = Low16bits(dataOnRead);
        }
    }
    
}

/* Assert if SR will be valid */
if(PS.MEM_V) {
   if (!dCacheEN) {
        NEW_PS.SR_V = 1; /* Not a LOAD/STORE OP */
   } else {
        if (dCacheR) {
            NEW_PS.SR_V = 1; /* Accessed and Ready */
        } else {
            NEW_PS.SR_V = 0;
        }
   }     
} else {
    NEW_PS.SR_V = 0; /* If Current MEM is not valid, pass the bubble */
}

         
    int brOP = Get_BR_OP(PS.MEM_CS);
    int uncondOP = Get_UNCOND_OP(PS.MEM_CS);
    int trapOP = Get_TRAP_OP(PS.MEM_CS);
    int n = get_bits(PS.MEM_IR, 11,11);
    int z = get_bits(PS.MEM_IR, 10, 10);
    int p = get_bits(PS.MEM_IR, 9 , 9);
    int ldCC = Get_MEM_LD_CC(PS.MEM_CS);
    int ldReg = Get_MEM_LD_REG(PS.MEM_CS);
    int brStall = Get_MEM_BR_STALL(PS.MEM_CS); 

    if (PS.MEM_V) {
        v_mem_ld_cc = ldCC;
        v_mem_ld_reg = ldReg;
        v_mem_br_stall = brStall;
    } else {
        v_mem_ld_cc = 0;
        v_mem_ld_reg = 0;
        v_mem_br_stall = 0;
...
if (PS.MEM_V) {
    ...
    memPCMUX = 0; 
    memTARGETPC = Low16bits(PS.MEM_ADDRESS);
    
    if (uncondOP) {
        memPCMUX = 1;
    } else if (trapOP) {
        memPCMUX = 2;
        memTRAPPC = Low16bits(dataOnRead);
    } else if (brOP) {
        if ((n & get_bits(PS.MEM_CC, 2, 2)) || (z & get_bits(PS.MEM_CC, 1, 1)) || 
                (p & get_bits(PS.MEM_CC, 0, 0))) {
                    memPCMUX = 1;
        }
    }
}
    ...
    if (dCacheEN && PS.MEM_V) {
        NEW_PS.SR_DATA = Low16bits(dataOnRead);
    } else {
        NEW_PS.SR_DATA = 0;
    }
...
void AGEX_stage() {
  ...
  int LD_MEM; /* You need to write code to compute the value of LD.MEM
		 signal */
    LD_MEM = !mem_stall; /* If stalled, then don't write to memory latch! */


  /* your code for AGEX stage goes here */
    int SR1Val = PS.AGEX_SR1;
    int SR2Val = PS.AGEX_SR2;
    int aluk = Get_ALUK(PS.AGEX_CS);
    int SR2MUX = Get_SR2MUX(PS.AGEX_CS); 
    int aluRESMUX = Get_ALU_RESULTMUX(PS.AGEX_CS);
    int imm5 = SEXT(get_bits(PS.AGEX_IR, 4 , 0),5);
    int amount4 = get_bits(PS.AGEX_IR, 3, 0);    
    int bit4 = get_bits(PS.AGEX_IR, 4, 4);
    int bit5 = get_bits(PS.AGEX_IR, 5, 5);
   ...
   /* ALU Calculations here */ 

    if (aluRESMUX) {
       if (aluk == 0) {
            ALURESULT = Low16bits(SR1Val + B);    
        } else if (aluk == 1) {
            ALURESULT = Low16bits(SR1Val & B);
      } else if (aluk == 2) {
         ALURESULT = Low16bits(SR1Val ^ B);
      } else {
         ALURESULT = Low16bits(B);
       }
    } else {
        
        if (!bit4) {
            ALURESULT = Low16bits(SR1Val << amount4);        
        } else {
            if (bit5) {
                ALURESULT = RSHF(SR1Val, amount4, get_bits(SR1Val, 15, 15));
            } else {
                ALURESULT = RSHF(SR1Val, amount4, 0);
            }
        }
    }
   /* SHF Calculations here */

  
    int addr1MUX = Get_ADDR1MUX(PS.AGEX_CS);
    int addr2MUX = Get_ADDR2MUX(PS.AGEX_CS);
    int addrMUX = Get_ADDRESSMUX(PS.AGEX_CS);
    ...
    if (addr1MUX) {
        ...
        addr1Val = PS.AGEX_NPC;
    }
    
    if (addr2MUX == 0) {
        addr2Val = 0;
    } else if (addr2MUX == 1) {
        addr2Val = SEXT(get_bits(PS.AGEX_IR, 5, 0),6);
    } else if (addr2MUX == 2) {
        addr2Val = SEXT(get_bits(PS.AGEX_IR, 8, 0),9);
    } else {
        addr2Val = SEXT(get_bits(PS.AGEX_IR, 10, 0),11);
    }
    
    addr2Val = addr2Val << lshf1;
    addSum = addr2Val + addr1Val;  
    
    if (addrMUX) {
        finalAddress = Low16bits(addSum);
    } else {
        finalAddress = Low16bits((PS.AGEX_IR & 0xFF) << 1);
    }
  ...
  if (LD_MEM) {
    ...
    NEW_PS.MEM_ADDRESS = finalAddress;
...
void DE_stage() {
  ...
  /* your code for DE stage goes here */
    ...
    int IR1511 = get_bits(PS.DE_IR, 15, 11);
    int IR5 = get_bits(PS.DE_IR, 5, 5);
    CONTROL_STORE_ADDRESS = (IR1511 << 1) | IR5;   
    
    int SR1Ind = get_bits(PS.DE_IR, 8 , 6);
    int SR1Need = Get_SR1_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS]);

    int SR2IDMUX = get_bits(PS.DE_IR, 13, 13);
    int SR2Need = Get_SR2_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS]);
    int SR2Ind;
    if (SR2IDMUX) {
        SR2Ind = get_bits(PS.DE_IR, 11, 9);   
    } else {
        SR2Ind = get_bits(PS.DE_IR, 2, 0);
    }
    int DRMUX = Get_DRMUX(CONTROL_STORE[CONTROL_STORE_ADDRESS]);
    int DR;
    int BranchOp = Get_DE_BR_OP(CONTROL_STORE[CONTROL_STORE_ADDRESS]);
    int BR_STALL = Get_DE_BR_STALL(CONTROL_STORE[CONTROL_STORE_ADDRESS]);
    int setCC = (N << 2) | (Z << 1) | (P);
    ...
    int SR1Val = REGS[SR1Ind];
    int SR2Val = REGS[SR2Ind];
...
if (PS.DE_V) {
  ...
  if (LD_AGEX) {
    ...
    NEW_PS.AGEX_SR1 = SR1Val;
    NEW_PS.AGEX_SR2 = SR2Val;
...
void FETCH_stage() {
  ...
  /* your code for FETCH stage goes here */
    ...
    int IRtoRead;
    icache_access(PC, &IRtoRead, &icache_r);
    ...
    DEValid = icache_r && !(v_de_br_stall || v_agex_br_stall || v_mem_br_stall);

    if (icache_r && !dep_stall && !mem_stall && !(v_de_br_stall || v_agex_br_stall)) {
	LD_PC = 1;

	if (v_mem_br_stall) {
		if (memPCMUX == 1 || memPCMUX == 2){
			LD_PC = 1;
		} else {
			LD_PC = 0;
		}
    ...
    if (LD_PC) {    
      ...
      if (memPCMUX == 1 && PS.MEM_V) {
            PC = memTARGETPC;
      } else if (memPCMUX == 2 && PS.MEM_V) {
            PC = memTRAPPC;
      } else {
